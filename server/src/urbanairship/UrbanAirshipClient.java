...
public class UrbanAirshipClient {
	...
	public void register(Device dev) {
		...
		put("/api/" + path + "/" + encode(identifier), dev);
	}
	...
	public void unregister(Device dev) {
		...
		delete("/api/" + path + "/" + encode(identifier));
	}
	...
	public void addTagToDevice(Device device, String tag) {
		...
		put("/api/device_tokens/" + encode(device.getIdentifier()) + "/tags/"
				+ encode(tag));
	}
	...
	public FeedbackList getFeedback(final Calendar since) {
		FeedbackList feedbackList = get(FeedbackList.class,
